...
sudo ip address add 192.168.255.9/31 dev toborder
...
# Add masquerade rule for traffic from borderleaf (192.168.255.8) going to internet (idempotent)
# Traffic from all VRFs will be SNATed to 192.168.255.8 on borderleaf, then masqueraded here to host IP
if ! sudo iptables -t nat -C POSTROUTING -s 192.168.255.8 ! -o toborder -j MASQUERADE 2>/dev/null; then
    sudo iptables -t nat -A POSTROUTING -s 192.168.255.8 ! -o toborder -j MASQUERADE
